...
protected:
  ...
  {      
    ...
    std::string strIdentifier_1 {"client_1"};
    std::string strIdentifier_2 {"client_2"};

    pCommander->Register(strIdentifier_1);
    pCommander->Register(strIdentifier_2);

    std::string strLine;
    while ( std::getline(ssTestIn, strLine) ) {
      pCommander->ProcessLine(strIdentifier_1, strLine);
      pCommander->ProcessLine(strIdentifier_2, strLine);
    }    

    std::this_thread::sleep_for(std::chrono::seconds(a_szDelay));
    
    pCommander->Unregister(strIdentifier_1);
    pCommander->Unregister(strIdentifier_2);
  }
...
TEST_F(Bulkmt_2_Clinets_Test, not_empty_stream) { 
  ssTestIn << "cmd1" << std::endl
    << "cmd2" << std::endl
    << "cmd3" << std::endl
  ;
  ...
  ASSERT_EQ(ssMetricsMain.str(), "main: 6 line(s), 2 block(s), 6 command(s)\n");
  ASSERT_EQ(ssMetricsLog.str(), "log: 2 block(s), 6 command(s)\n");
}

TEST_F(Bulkmt_2_Clinets_Test, single_block) { 
  ssTestIn << "{" << std::endl
    << "cmd1" << std::endl
    << "cmd2" << std::endl
    << "cmd3" << std::endl
  ...
  ASSERT_EQ(ssMetricsLog.str(), "log: 2 block(s), 10 command(s)\n");
}

TEST_F(Bulkmt_2_Clinets_Test, multi_block) { 
  ssTestIn << "cmd1" << std::endl
    << "cmd2" << std::endl
    << "{" << std::endl
    << "cmd1.1" << std::endl
  ...
  ASSERT_EQ(ssTestOut.str(), "bulk: cmd1, cmd1, cmd2\n"
    ...
    "cmd1.4, cmd1.5\n"
    "bulk: cmd2\n");
  ASSERT_EQ(ssMetricsMain.str(), "main: 32 line(s), 4 block(s), 24 command(s)\n");
  ASSERT_EQ(ssMetricsLog.str(), "log: 4 block(s), 24 command(s)\n");
}
